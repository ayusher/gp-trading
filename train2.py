...
class GeneticStrategy:
    ...
    def trade(self, data, buying_power, assets):
        ...
        try:
            if out > 0:
                #print(min(1, out/100))
                return {ticker: ("buy", int((buying_power*min(1, out/100))//np.array(data[ticker]["Close"])[-1]))}
            elif out < 0:
                return {ticker: ("sell", assets[ticker][0]*min(1, abs(out)/100)) }
...
def genetic_algorithm():
    ...
    tickers = ["IBM"] #["IBM", "GOOG", "MSFT", "AAPL", "COST", "GME"]
    ...
    while True:
        g+=1
        ticker = tickers[g%len(tickers)]
        backtest = Backtest(tickers={ticker: pd.read_csv(ticker+".csv")[:2000]}, interval="1d")
        backtest_test = Backtest(tickers={ticker: pd.read_csv(ticker+".csv")[2000:].reset_index()}, interval="1d")
        ...
        for genome in population:
            ...
            li.append(GeneticStrategy([ticker], "1d", genome).trade)
        ...
        best_fitnesses.append(temp[0][0][0] - temp[0][0][1]/1000)
        avg_fitnesses.append(sum([x[0][0] - x[0][1]/1000 for x in temp])/len(temp))
        plt.clf()
